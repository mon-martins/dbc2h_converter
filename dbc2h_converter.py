...
if(len(sys.argv) < 4): raise Exception("missing parameter, usage: directory_of_dbc name_of_dbc_file system_name")
...
for message in dbc_inst.messages:
    ...
    header.write( "/************************************************************/\n")
    header.write(f"#define CAN_MSG_{message.name}\n")
    ...
    for signal in message.signals:
        ...
        header.write(f"    {sig_type} CAN_SIG_{signal.name}:{signal.length};\n")
    ...
    for signal in message.signals:
        ...
        header.write(f"#define CAN_SIG_{signal.name}\n")
        if signal.is_signed:
